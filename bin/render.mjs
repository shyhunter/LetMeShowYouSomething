...
@media (max-width:520px){#topbar:has(.home){flex-wrap:wrap}.home~#start-mini{order:3;flex-basis:100%}#topbar .theme{display:none}}
...
function galleryHtml(nowScreen){
  if (!FLOW) return '';
  return '<div class="gallery" tabindex="0" role="region" aria-label="Every screen, in order">' + FLOW.screens.map((s, i) => '<div class="gcard' + (s.id === nowScreen ? ' now' : '') + '" data-screen="' + esc(s.id) + '">' + appHtml(s.id) + '<p><span class="step">' + (i + 1) + '</span>' + esc(s.title) + '</p></div>').join('') + '</div>';
}
