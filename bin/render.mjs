...
const ICONS = {
  ...
  send: '<path d="M4 12l16-8-6 16-3-7z"/>', sun: '<circle cx="12" cy="12" r="4"/><path d="M12 2v2M12 20v2M4.9 4.9l1.4 1.4M17.7 17.7l1.4 1.4M2 12h2M20 12h2M4.9 19.1l1.4-1.4M17.7 6.3l1.4-1.4"/>', moon: '<path d="M21 12.8A9 9 0 1 1 11.2 3a7 7 0 0 0 9.8 9.8z"/>', home: '<path d="M3 11l9-8 9 8"/><path d="M5 10v10h14V10"/><path d="M10 20v-6h4v6"/>', eye: '<path d="M2 12s4-7 10-7 10 7 10 7-4 7-10 7S2 12 2 12z"/><circle cx="12" cy="12" r="3"/>',
...
// #147 — day or night: a switch shown as a sun and a moon. It starts from the system's setting; a choice is kept in
// this browser only, never in the answers or the export. Hidden until the script runs, as it does nothing without one.
const themeSwitch = `<button type="button" class="theme" role="switch" aria-checked="false" aria-label="Dark mode" title="Day or night" data-theme-switch hidden>${I('sun', 'sm')}${I('moon', 'sm')}</button>`;
...
${favicon}
<script>try { const t = localStorage.getItem('lmsys:theme'); if (t === 'light' || t === 'dark') document.documentElement.dataset.theme = t; } catch {}</script>
<style>
...
.start-bar{display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:14px}
.start-bar:not(:has(.home)){justify-content:flex-end}
.theme{--k:26px;position:relative;flex:none;display:inline-flex;align-items:center;justify-content:space-between;width:66px;height:34px;padding:0 7px;border:2px solid var(--edge);border-radius:99px;background:var(--sunk);color:var(--ink);cursor:pointer;box-shadow:2px 2px 0 var(--edge)}
.theme[hidden]{display:none}
.theme .ic{position:relative;z-index:1}
.theme::after{content:"";position:absolute;top:2px;left:2px;width:var(--k);height:var(--k);border-radius:50%;background:var(--surf);border:2px solid var(--edge);box-sizing:border-box;transition:left .2s}
.theme[aria-checked="true"]::after{left:calc(100% - var(--k) - 2px)}
.theme:active{transform:translate(2px,2px);box-shadow:none}
@media (pointer:coarse){.theme{--k:36px;width:84px;height:44px;padding:0 11px}}
@media (prefers-reduced-motion:reduce){.theme::after{transition:none}}
...
@media (max-width:520px){#start-mini .more{display:none}#topbar:has(.home){flex-wrap:wrap}.home~#start-mini{order:3;flex-basis:100%}#topbar .theme{display:none}}
...
/* #146 — a desktop screen: a browser window, wide; buttons in a row, a dialog in the middle. It narrows to fit a phone. */
.app.desk{max-width:760px;min-height:360px;border-width:3px;border-radius:12px;grid-template-rows:auto auto 1fr auto;font-size:13.5px}
.app.desk.shot{min-height:0}
.app .win{display:flex;align-items:center;gap:10px;padding:7px 10px;background:#ECEEF1;border-bottom:1px solid #DDE1E6}
.app .win .lights{display:inline-flex;gap:5px;flex:none}
.app .win .lights i{width:9px;height:9px;border-radius:50%;background:#C9CED6}
.app .win .url{flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;background:#FFFFFF;border-radius:7px;padding:3px 10px;font:500 11.5px var(--sans);color:#4B5260;text-align:center}
.app.desk .bar{grid-template-columns:28px 1fr 28px;padding:8px 16px}
.app.desk .bar b{text-align:left;font-size:15px}
.app.desk .body{padding:16px 18px;grid-template-columns:repeat(auto-fit,minmax(min(220px,100%),1fr));gap:12px}
.app.desk .body>:not(.c-card){grid-column:1/-1}
.app.desk .foot{display:flex;flex-wrap:wrap;justify-content:flex-end;gap:8px;padding:10px 16px}
.app.desk .pri,.app.desk .sec{padding:8px 16px}
.app.desk .scrim{align-items:center;justify-content:center}
.app.desk .sheet{width:min(380px,90%);border-radius:14px}
.app.desk .grab{display:none}
.gcard.desk,.variant.desk{width:min(520px,86vw)}
.variant.desk .app{max-width:100%;min-height:300px}
...
.desk .shot-alt{aspect-ratio:16/10}
...
.shot-tag{position:absolute;left:8px;bottom:8px;display:inline-flex;gap:4px;align-items:center;background:#15131A;color:#fff;border-radius:999px;padding:3px 9px;font:600 11px var(--sans)}
.shot-tools{flex-basis:100%;display:flex;flex-wrap:wrap;gap:6px;align-items:center;justify-content:center;margin-top:10px}
...
.proto-wrap{display:flex;flex-wrap:wrap;justify-content:center}
...
@container (max-width:700px){
  ...
  .slots .app.desk{max-width:calc(100% / .6)}
...
.gcard.desk .app{max-width:100%;min-height:300px}
...
/* #129 — soft brutalist (style D): bold outlines and clearly bounded blocks; a hard shadow only on what can be pressed,
   so the look itself says what is clickable; the current question lifted in purple; our calm colours; heavy system
   type for headings, labels and buttons (no font file: the page fetches nothing and stays MIT-0). */
h1,h2,.t-head h2,.brief h2,#start h1{font-weight:800;letter-spacing:-.022em}
.eyebrow,.sec-name,.slot-head h3,.scard h3,.c-sec,.prog-sum,.pseg .pl{font-family:var(--sans);font-weight:800;letter-spacing:.07em}
.btn,.tile,.opt,#start-mini,.seg,.variant .btn{border:2px solid var(--edge)}
.btn:not(.quiet),.tile,.opt,#start-mini{box-shadow:2px 2px 0 var(--edge);font-weight:700}
.btn.pri{border-color:var(--edge);box-shadow:3px 3px 0 var(--edge)}
.btn:not(.quiet):not(:disabled):active,.tile:active,.opt:active,#start-mini:active{transform:translate(2px,2px);box-shadow:none}
.btn.quiet{border:0;box-shadow:none;text-decoration:underline;text-underline-offset:3px;text-decoration-thickness:2px}
.btn.icon{border-color:var(--edge)}
.seg{background:var(--surf);box-shadow:2px 2px 0 var(--edge)}
.seg button[aria-pressed="true"]{background:var(--ac);color:var(--on-ac);box-shadow:none}
.tile{border-radius:10px}
.tile .dot{border:1.5px solid var(--edge)}
.tile:has(input:checked){border-color:var(--edge);box-shadow:3px 3px 0 var(--edge)}
.opt[aria-pressed="true"]{border-color:var(--edge);box-shadow:4px 4px 0 var(--lift)}
.scard,.slot,.t-head,.t-ans,.c-item,.brief,.dlf,.settled,.hround,.preview,.savefile,.addmissing,.added>div,.card{border:2px solid var(--edge)}
.exp,.sum-row{border:1.5px solid var(--edge)}
.c-item{border-left:6px solid var(--sc,var(--edge))}
.t-head{border-top:6px solid var(--sc,var(--ac))}
.t-ans{border-top:0}
.t-head{box-shadow:6px 0 0 var(--lift)}
.t-ans{box-shadow:6px 6px 0 var(--lift)}
.scard .num,.slot-head .num{border:2px solid var(--edge);font-family:var(--sans);font-weight:800}
.slot-head{border-bottom:2px solid var(--edge)}
.chip-part,.status,.tag{border:1.5px solid currentColor}
.explain textarea,.mark textarea,.addmissing input,.addmissing textarea{border:2px solid var(--edge)}
.pseg .track{border:1.5px solid var(--edge);height:10px}
.pseg.now .track{outline:3px solid var(--lift);outline-offset:2px}
.dg .dg-shape{stroke:var(--edge);stroke-width:2}
.dg .dg-edge path{stroke:var(--edge);stroke-width:2}
.dg marker path{fill:var(--edge)}
.dg .dg-selected .dg-shape{stroke:var(--lift);stroke-width:3.5}
.dg .dg-answered .dg-shape{stroke:var(--ok);stroke-width:2.5}
...
<section id="start" aria-labelledby="start-h">
  <div class="start-bar">${homeLink}${themeSwitch}</div><p class="eyebrow">${I('flag', 'sm')}Let me explain${view ? ` · round ${view.round}` : ''}</p>
...
// The day or night choice (#147) is this reader's, applied before this line runs: the copy never carries it.
const PAGE = '<!doctype html>\\n' + document.documentElement.outerHTML.replace(/^(<html[^>]*?) data-theme="(?:light|dark)"/, '$1');
...
const SPARE_ICONS = ['alert', 'clock', 'flag', 'pin', 'star', 'info'];
...
function picsHtml(on){
  return '<div class="pics" data-pics="' + esc(on) + '">' + (store.pictures || []).filter(p => p.on === on && !p.screen).map(p => '<figure class="pic"><img alt="Your picture" src="' + picSrc(p)
...
async function addPicture(on, file, screen){
  const msg = (t) => { const m = document.querySelector(screen ? '.shot-tools .pic-msg' : '[data-pics="' + CSS.escape(on) + '"] .pic-msg'); if (m) m.textContent = t; };
  const kept = (store.pictures || []).filter(x => !(screen && x.screen === screen));
  if (!file || !PIC_TYPES.includes(file.type)) return msg('Only PNG, JPEG or WebP pictures.');
  if (kept.length >= PIC_COUNT) return msg('At most ' + PIC_COUNT + ' pictures in one answer.');
  ...
  if (kept.reduce((n, x) => n + picBytes(x), picBytes(p)) > PIC_TOTAL) return msg('Your pictures together are too large. Remove one first.');
  p.id = 'picture-' + (Math.max(0, ...(store.pictures || []).map(x => +String(x.id).split('-')[1] || 0)) + 1);
  if (screen) { p.screen = screen; store.pictures = kept.concat(p); save(); render(null); return msg('Your screenshot shows this screen now. Re-saved without hidden details such as location.'); }
...
let picFor = null, picScreen = null;
picInput.addEventListener('change', () => { const f = picInput.files[0]; picInput.value = ''; if (f && picFor) addPicture(picFor, f, picScreen); });
...
// #146 — what a screen is shown on: its own device, else the flow's, else a phone.
const deviceOf = (s, flow = FLOW) => (s && s.device) || (flow && flow.device) || 'phone';
...
// Parts answered in an earlier round and not asked again: a box on one is marked answered.
const EARLIER_ITEMS = HISTORY ? HISTORY.rounds.flatMap(r => (r.review.items || []).map(i => i.id)).filter(id => !REVIEW.items.some(i => i.id === id)) : [];
...
function mapHtml(s, zoom, d){
  ...
  const answered = new Set(REVIEW.items.filter(i => store.verdicts[i.id] || Object.values(store.choices).includes(i.id)).map(i => i.id).concat(EARLIER_STEPS.map(i => i.id), EARLIER_ITEMS));
...
function shotHtml(s, on, live, mine){
  ...
  return '<div class="app shot' + (deviceOf(screenOf(s.id) || s) === 'desktop' ? ' desk' : '') + '"><div class="shot-in">' + (SHOT.test(s.image.src) ? '<img src="' + esc(s.image.src) + '" alt="' + esc(s.image.alt) + '">' : '<p class="shot-alt">' + esc(s.image.alt) + '</p>') + (mine ? '<span class="shot-tag">' + I('image', 'sm') + 'Your screenshot</span>' : '') + areas + '</div></div>';
}
function appHtml(screenId, on, live, flow = FLOW){
  ...
  const desk = deviceOf(s, flow) === 'desktop';
  return '<div class="app' + (desk ? ' desk' : '') + '" role="img" aria-label="The screen: ' + esc(s.title) + (desk ? ', on a computer' : '') + '">'
    + (desk ? '<div class="win"><span class="lights"><i></i><i></i><i></i></span><span class="url">' + esc(s.title) + '</span></div>' : '<div class="sb"><span>9:41</span><span>' + I('wifi', 'sm') + I('battery', 'sm') + '</span></div>')
...
function galleryHtml(nowScreen){
  if (!FLOW) return '';
  return '<div class="gallery" tabindex="0" role="region" aria-label="Every screen, in order">' + FLOW.screens.map((s, i) => '<div class="gcard' + (s.id === nowScreen ? ' now' : '') + (deviceOf(s) === 'desktop' ? ' desk' : '') + '" data-screen="' + esc(s.id) + '">' + appHtml(s.id) + '<p><span class="step">' + (i + 1) + '</span>' + esc(s.title) + '</p></div>').join('') + '</div>';
}
...
function examplesRows(list){
  return (list || []).map(x => exp('info', 'globe', 'Done before · ' + esc(x.name), esc(x.what || '') + (x.shows ? ' What people see: ' + esc(x.shows) + '.' : '')
    + ' ' + (isWebSource(x.source) ? '<a href="' + esc(x.source) + '" target="_blank" rel="noopener noreferrer">' + esc(String(x.source).replace(/^https?:\\/\\//, '').split('/')[0]) + '</a>' : '<span class="unverified">unverified · I could not find a source</span>'))).join('');
}
// #33 — the reviewer can show the screen as it really is: their screenshot replaces it and goes back in the file.
function shotTools(it){
  const sid = it.step.from, mine = (store.pictures || []).find(p => p.screen === sid);
...
function variantsHtml(s){
  ...
  return '<div class="variants">' + cards.map(([it, to]) => '<div class="variant' + (pick === it.id ? ' on' : '') + (deviceOf(screenOf(to)) === 'desktop' ? ' desk' : '') + '">' + appHtml(to)
...
function slotHtml(id, label, icon, i, body, sub, tab){
  ...
  return '<section class="slot' + (min ? ' min' : '') + (shown ? ' shown' : '') + '" id="slot-' + id + '" data-slot="' + id + '"><div class="slot-head"><span class="num">' + (i + 1) + '</span><h3>' + I(icon, 'sm') + label + (sub ? ' <small>· ' + esc(sub) + '</small>' : '') + '</h3>'
    ...
    + '<div class="slot-body" tabindex="0" role="region" aria-label="' + label + '">' + (body || '<p class="empty">' + I('info', 'sm') + 'Nothing here for this question</p>') + '</div></section>';
}
// Only the places this question has something in are shown: no empty places.
function slots(bodies, subs){
  subs = subs || {};
  const list = SLOTS.filter(([id]) => bodies[id]);
  if (!list.length) return '';
  const tab = list.some(([id]) => id === st.ptab) ? st.ptab : list[0][0];
  const onlyProto = list.some(([id]) => id === 'proto') && list.every(([id]) => id === 'proto' || st.min.has(id)) && !st.min.has('proto');
  return (list.length > 1 ? '<div class="right-head"><span class="prog-sum">' + I('layers', 'sm') + ' ' + (list.length === 4 ? 'The same four places on every question' : 'What there is to see') + '</span><div class="rowgap">'
    + (list.some(([id]) => id === 'proto') ? '<button type="button" class="btn small' + (onlyProto ? ' pri' : '') + '" data-act="focus-proto" aria-pressed="' + onlyProto + '">' + I('eye', 'sm') + 'Prototype only</button>' : '') + '<button type="button" class="btn small" data-act="focus-all">Show all</button></div></div>'
    + '<div class="ptabs" role="group" aria-label="What to show">' + list.map(([id, label, icon]) => '<button type="button" data-ptab="' + id + '" aria-pressed="' + (tab === id) + '">' + I(icon, 'sm') + (id === 'expected' ? 'Should happen' : id === 'build' ? 'Build' : id === 'proto' ? 'Screen' : label) + '</button>').join('') + '</div>' : '')
    + '<div class="slots">' + list.map(([id, label, icon], i) => slotHtml(id, label, icon, i, bodies[id], subs[id], tab)).join('') + '</div>';
}
...
// ── the tour ──
function navHtml(prevOff, nextLabel, nextIcon){
  const n = STEPS.length + 2, now = st.phase === 'return' ? n : st.cur + 2;
  return '<div class="t-nav"><button type="button" class="btn" id="back" data-act="prev"' + (prevOff ? ' disabled' : '') + '>' + I('left', 'sm') + 'Back</button><span class="prog-sum" id="stepno">Step ' + now + ' of ' + n + '</span><button type="button" class="btn pri" id="next" data-act="next">' + (nextIcon ? I(nextIcon, 'sm') + nextLabel : nextLabel + I('right', 'sm')) + '</button></div>';
}
function renderTour(){
...
const next = () => st.phase === 'return' ? exportAs('html') : st.cur < STEPS.length - 1 ? go('q', st.cur + 1) : go('return');
const prev = () => st.phase === 'return' ? go('q', STEPS.length - 1) : st.cur > 0 ? go('q', st.cur - 1) : go('start');
function exportAs(fmt){
  const out = buildFeedback(REVIEW, store);
  if (fmt === 'json') saveAs(JSON.stringify(out, null, 2), 'application/json', REVIEW.id + '.feedback.json');
  else if (fmt === 'md') saveAs(toMarkdown(), 'text/markdown', REVIEW.id + '.feedback.md');
  else saveAs(htmlSeed(), 'text/html', REVIEW.id + '.feedback.html');
  const msg = 'Downloaded ' + fmt.toUpperCase() + ': ' + out.summary.answered + '/' + out.summary.total + ' answered · ' + out.summary.added + ' added · ' + out.gaps.length + ' gaps. Nothing was sent: return the file yourself.';
  $('#footnote').textContent = msg; const s = $('#download-status'); if (s) s.textContent = msg;
}
...
document.addEventListener('click', e => {
  ...
  if (d.export) return exportAs(d.export);
  if (t.id === 'addbtn') {
...
render(null);
// #147 — the day and night switch: checked means dark.
(() => {
  const dark = matchMedia('(prefers-color-scheme: dark)'), root = document.documentElement;
  const isDark = () => root.dataset.theme ? root.dataset.theme === 'dark' : dark.matches;
  const show = () => document.querySelectorAll('[data-theme-switch]').forEach(b => { b.hidden = false; b.setAttribute('aria-checked', String(isDark())); });
  document.querySelectorAll('[data-theme-switch]').forEach(b => b.addEventListener('click', () => {
