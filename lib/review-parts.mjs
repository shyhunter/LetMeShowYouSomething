...
// Only a web address becomes a link: anything else (a note, a javascript: URL) is shown as unverified.
export function isWebSource(u) { return /^https?:\/\/[^\s"<>]+$/i.test(String(u || '')); }
...
export function startCardsHtml(review, parts, icon, view) {
  ...
  const exRows = (b.examples || []).map((x) => '<li><b>' + partsEsc(x.name) + '</b> ' + partsEsc(x.what || '') + (x.shows ? ' What people see: ' + partsEsc(x.shows) : '') + ' '
    + (isWebSource(x.source) ? '<a href="' + partsEsc(x.source) + '" target="_blank" rel="noopener noreferrer">' + partsEsc(String(x.source).replace(/^https?:\/\//, '').split('/')[0]) + '</a>' : '<span class="unverified">unverified · I could not find a source</span>') + '</li>').join('');
  const cards = [
