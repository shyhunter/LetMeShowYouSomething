...
export function partLabel(diagram, on) {
  ...
  const e = (diagram?.edges || []).find((x) => x.from === on?.edge?.from && x.to === on?.edge?.to);
  return e ? `${name(e.from)} → ${name(e.to)}${e.label ? ` (${e.label})` : ''}` : null;
}
...
export function applyProposals(diagram, proposals) {
  ...
  for (const p of proposals) {
    ...
    const edge = d.edges.find((x) => x.from === p.from && x.to === p.to);
    ...
    if (p.op === 'rename') {
      ...
      else { edge.label = p.text; edge.kind = edge.kind ?? 'conditional'; }
...
export function buildFeedback(review, store, now = new Date().toISOString()) {
  ...
  const comments = (store.comments || []).filter((c) => (c.note || '').trim()).map((c) => {
    ...
    if (c.node !== undefined) o.node = c.node; else o.edge = { from: c.edge.from, to: c.edge.to };
  ...
  const proposals = (store.proposals || []).map((p) => {
    ...
    for (const k of ['node', 'from', 'to', 'text']) if (p[k] !== undefined) o[k] = p[k];
