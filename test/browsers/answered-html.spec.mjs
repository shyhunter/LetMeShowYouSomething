...
const save = (page, fmt, path) => download(page, fmt, path);
const open = async (page, url) => { await page.goto(url); await overview(page); };
...
test('#78: an exported page, opened and exported again, reads back as the same feedback', async ({ page }) => {
  ...
  await overview(page);                                               // every question on one page
  ...
  const first = await save(page, 'html', join(dir, 'answered.html'));
  const json = JSON.parse(readFileSync(await save(page, 'json', join(dir, 'feedback.json')), 'utf8'));
  await open(page, pathToFileURL(first).href);
  await expect(page.locator('#n-guest-checkout')).toHaveValue(note);
  await expect(page).not.toHaveTitle('ran');
  const second = await save(page, 'html', join(dir, 'answered-again.html'));
  await open(page, pathToFileURL(second).href);
  await expect(page.locator('#n-guest-checkout')).toHaveValue(note);
