...
const DPANEL = `    <section id="dpanel" class="panel" aria-label="The diagram">
      <div class="panel-head min-head">
        <span class="panel-title">How this works</span>
        ...
        <details class="panel-tools" id="diagram-tools"><summary>Diagram tools</summary><div class="tool-content">
...
.review-start{display:flex;gap:16px;align-items:center;flex-wrap:wrap;margin-top:20px}
.review-start .btn{font-size:14px;padding:11px 20px}
.panel-tools>summary{cursor:pointer;min-height:44px;display:list-item;padding:10px 12px;font-size:12.5px;color:var(--ink2)}
.panel-tools[open]{flex-basis:100%;border:1px solid var(--line2);border-radius:8px;background:var(--surf2)}
.tool-content{display:flex;align-items:center;gap:8px;flex-wrap:wrap;padding:8px 12px}
.tool-content .sizes{display:flex;gap:6px}
#brief{margin:0 0 24px}
#workspace{display:grid;gap:18px;grid-template-columns:minmax(0,1fr);margin-top:22px;align-items:start}
#workspace>.selection-head{grid-column:1/-1;border-bottom:1px solid var(--line);padding-bottom:12px}
#selection-title{font-size:20px;overflow-wrap:anywhere}
#selection-meta,#diagram-selection{font-size:13px;color:var(--mut);margin:6px 0}
#related-items{display:flex;gap:8px;flex-wrap:wrap}
#workspace #upanel,#workspace #feedback{margin-top:0;min-width:0}
#workspace #detailbox{position:static}
/* A pinned screen joins the page's vertical pin stack. The grid must not stop its sticky
   position at the workspace's bottom before the other pinned panels reach theirs. */
#workspace:has(>#upanel.pinned){display:contents}
#workspace .split{grid-template-columns:minmax(0,1fr)}
#workspace .split>.sizes{grid-column:1/-1}
#workspace #detail{scroll-margin-top:80px}
#workspace #steplist{max-height:300px;overflow:auto}
#workspace:has(#upanel) #steplist{max-height:210px}
#workspace:has(#upanel) #feedback{padding:16px}
.selection-actions{display:flex;gap:8px;flex-wrap:wrap;margin-block:8px}
.response-heading{font:600 13px var(--sans);margin:18px 0 8px;color:var(--ink)}
.response-extra{margin-top:12px}
.response-extra>summary{cursor:pointer;min-height:44px;padding:10px 0;color:var(--ink3);font-size:13px}
.done{z-index:7;gap:14px;padding:12px 8px;margin-top:24px;box-shadow:0 -8px 20px var(--bg)}
#review-progress{flex:1;font:13px var(--mono);color:var(--ink2)}
...
#return-review .summary-counts{display:flex;gap:12px;flex-wrap:wrap;padding:14px 0;border-block:1px solid var(--line);font:13px var(--mono)}
#return-review li{margin:6px 0;overflow-wrap:anywhere}
#return-review .summary-actions{display:flex;gap:10px;flex-wrap:wrap;margin-top:18px}
...
@media(min-width:1100px){
  #workspace:has(#upanel){grid-template-columns:minmax(0,.9fr) minmax(0,1.1fr)}
  #workspace:not(:has(#upanel)) .split{grid-template-columns:minmax(0,1fr) minmax(0,1fr)}
  #workspace:not(:has(#upanel)) .split[data-size="list"]{grid-template-columns:minmax(0,2fr) minmax(0,1fr)}
  #workspace:not(:has(#upanel)) .split[data-size="detail"]{grid-template-columns:minmax(0,1fr) minmax(0,2fr)}
  #workspace:has(#upanel):has(.split[data-size="list"]){grid-template-columns:minmax(0,1.2fr) minmax(0,1fr)}
  #workspace:has(#upanel):has(.split[data-size="detail"]){grid-template-columns:minmax(0,.7fr) minmax(0,1.3fr)}
}
@media(max-width:699px){.wrap.frame{padding-inline:12px}.done{position:static}.done .panel-tools{flex-basis:100%}#return-review{padding:18px}.panel-title{white-space:normal}.tool-content{padding:6px}.review-start{gap:8px}#workspace #steplist{max-height:240px}}
</style>
</head><body>
<a class="skip" href="#items">Skip to the items</a>
<div class="wrap frame">
<header>
  <p class="review-kicker">Understand · Inspect · Decide · Explain · Return</p>
  <div class="htop min-head">
    ...
    <div class="htools">
      <details class="panel-tools" id="display-tools"><summary>Display &amp; header tools</summary><div class="tool-content">
  ...
  <div class="review-start"><button class="btn pri" type="button" id="start-review">Start review</button><span>${review.items.length} items to review · Your answers stay in this browser until you download a copy.</span></div>
</header>
...
${DPANEL}    <section id="upanel" class="panel" aria-label="The screen">
      <div class="panel-head min-head">
        <span class="panel-title">What the user sees</span>
        <details class="panel-tools" id="screen-tools"><summary>Screen tools</summary><div class="tool-content">
...
${review.brief ? `    <section id="brief" aria-labelledby="brief-h">
    ...
    <section id="feedback" aria-labelledby="fb-h">
      <div class="min-head sec-bar"><h2 id="fb-h">What should happen</h2><button class="btn min" type="button" data-min="#feedback" aria-expanded="true" aria-label="Minimise your feedback">Minimise</button></div>
      <p id="overview" class="hint" aria-live="polite"></p>
      <div id="split" class="split" data-size="even">
        <details class="panel-tools sizes"><summary>Layout tools</summary><div class="tool-content" role="group" aria-label="How the row is shared">
        ...
        <div id="detailbox"><div class="min-head sec-bar"><h3>Inspect &amp; respond</h3><button class="btn min" type="button" data-min="#detailbox" aria-expanded="true" aria-label="Minimise the open item">Minimise</button></div>
...
<div class="done">
  <output id="review-progress" aria-live="polite"></output>
  <button class="btn pri" id="finish" type="button">Finish review</button>
  <details class="panel-tools" id="review-tools"><summary>Review tools</summary><div class="tool-content">
  <button class="btn" id="reset" type="button">Clear my answers</button>
  </div></details>
  <span class="note" id="footnote">Answers save in this browser as you go. Downloading saves a file; nothing is sent from this page.</span>
</div>
...
<dialog id="return-review" aria-labelledby="return-heading">
  ...
  <p class="note">Return either downloaded file to the agent. JSON can be checked directly; the answered HTML can be read as data and checked with bin/answer.mjs.</p>
  <div class="summary-actions"><button class="btn pri" id="export" type="button">Download JSON for the agent</button><button class="btn" id="exporth" type="button">Download answered HTML</button><button class="btn" id="continue-review" type="button">Continue reviewing</button></div>
...
function render(){
  ...
  $('#review-progress').textContent = answered + ' / ' + REVIEW.items.length + ' answered';
  ...
  $('#detail').innerHTML = open ? itemHtml(open, (REVIEW.sections || []).find(s => s.id === open.sectionId), true)
    ...
    : '<p class="hint">Pick an item to inspect it and give your answer.</p>';
  renderSelection();
}
...
function stepRow(it, cur, sec){
  ...
  return \`<fieldset class="item row" data-v="\${cur}" data-step="\${esc(it.id)}" aria-current="\${selected === it.id}" style="--tone:var(--\${TONE[(o || {}).tone] || 'line2'})">
    ...
    <p class="row-meta">\${it.step ? \`<span class="w-journey">\${esc((JOURNEYS.find(j => j.id === journeyOf(it)) || {}).title || '')}</span>
      ...
      \${store.requests[it.id]?.example || store.requests[it.id]?.explain ? '<span>· request outstanding</span>' : ''}
...
function itemHtml(it, sec, full){
  ...
  if (!full) return stepRow(it, cur, sec);
  ...
  return \`<fieldset class="item\${chosen ? ' chosen' : ''}\${it.step ? ' open' : ''}" data-v="\${cur}"\${stepAttrs} style="--tone:var(--\${tone||'line2'})">
    ...
    <h3 class="response-heading">Your response · \${esc(it.title)}</h3>
...
$('#export').addEventListener('click', () => {
  ...
  $('#download-status').textContent = 'Downloaded JSON. Return this file to the agent; nothing was sent automatically.';
});
...
$('#exporth').addEventListener('click', () => {
  ...
  $('#download-status').textContent = 'Downloaded answered HTML. Return this file to the agent for checked import; nothing was sent automatically.';
});
...
function finishSummary(){
  ...
  $('#finish-summary').innerHTML = '<p>' + (partial ? 'Partial review — unanswered items and choices are kept, never treated as agreement.' : 'All items have an answer. Answers are not permission to act.') + '</p>'
...
$('#finish').addEventListener('click', () => { summaryDestination = false; finishSummary(); $('#download-status').textContent = ''; $('#return-review').showModal(); $('#return-heading').focus(); });
...
$('#finish-summary').addEventListener('click', e => {
  ...
  } else { selectTarget({ kind: 'added', id: b.dataset.summaryTarget }); revealRegion($('#detail')); $('#selection-title').focus(); }
});
...
let selection = !FLOW && REVIEW.items.length ? resolveReviewSelection(selectionReview, { kind: 'item', itemId: REVIEW.items[0].id }) : null;
let selected = selection?.itemId || null; // derived compatibility value for drawing; never saved
function selectTarget(target, keepDetail = false){
  ...
  selection = next; selected = next.itemId;
  at = next.screenId;
...
function renderSelection(){
  ...
  const it = REVIEW.items.find(i => i.id === selected);
  $('#selection-title').textContent = 'Selected: ' + (selection?.label || it?.title || 'choose something to inspect');
  $('#selection-meta').textContent = (selection?.layerKey ? 'Layer response: ' + selection.label + ' · Within: ' + it.title
    : it ? 'Your answer applies to: ' + it.title : selection?.target.kind === 'added' ? 'Your added feedback; not a verdict on another item.'
    : selection?.screenId ? 'Choose a related behavior below. This screen has no separate verdict.' : 'Selecting something does not record an answer.')
    + (it && !visible(it) ? ' · Selected item is outside the current filters.' : '');
  $('#related-items').innerHTML = !selected ? (selection?.relatedItemIds || []).map(id => '<button class="btn" type="button" data-open="' + esc(id) + '">' + esc(REVIEW.items.find(i => i.id === id)?.title) + '</button>').join('') : '';
  $('#selection-actions').innerHTML = (selection?.commentTarget ? '<button class="btn" id="comment-selected" type="button">Comment / propose a change</button>' : '')
    + (it ? '<button class="btn" id="inspect-response" type="button">Go to response</button>' : '');
  if (selection?.target.kind === 'added') {
...
function openItem(id){
  ...
  revealRegion($('#detail'));
  $('#detail input[type=radio]')?.focus({ preventScroll: true });
...
// The brief precedes inspection, and the selected screen and behavior form one workspace.
if ($('#brief')) $('header').after($('#brief'));
const workspace = document.createElement('section'); workspace.id = 'workspace'; workspace.setAttribute('aria-labelledby', 'selection-title');
workspace.innerHTML = '<div class="selection-head"><p class="review-kicker">Inspect together</p><h2 id="selection-title" tabindex="-1">Choose something to inspect</h2><p id="selection-meta"></p><div id="related-items"></div><div id="selection-actions" class="selection-actions"></div></div>';
$('#feedback').before(workspace);
if ($('#upanel')) workspace.append($('#upanel'));
workspace.append($('#feedback'));
$('#related-items').addEventListener('click', e => { const b = e.target.closest('[data-open]'); if (b) openItem(b.dataset.open); });
...
document.addEventListener('click', e => {
  ...
  if (added) { selectTarget({ kind: 'added', id: added.dataset.selectAdded }); revealRegion($('#detail')); $('#selection-title').focus(); }
...
$('#start-review').addEventListener('click', () => {
  for (const id of ['#dpanel', '#feedback', '#steplist', '#detailbox']) mins = mins.filter(x => x !== id);
  applyMins();
  const region = $('#dpanel') || $('#workspace');
  revealRegion(region);
  region.setAttribute('tabindex', '-1'); region.focus(); region.scrollIntoView({ block: 'start' });
});
function revealRegion(region){
  for (let el = region; el; el = el.parentElement) {
    if (el.classList.contains('sec-body')) { el.hidden = false; el.parentElement.querySelector('.sec-toggle')?.setAttribute('aria-expanded', 'true'); }
    if (el.classList.contains('minimised')) { mins = mins.filter(sel => $(sel) !== el); applyMins(); }
  }
}
function focusResponse(layerKey){
  revealRegion($('#detail'));
  const entry = layerKey && $('#detail input[data-lv="' + CSS.escape(layerKey) + '"]');
  if (entry) {
...
if (FLOW) {
  ...
  const pieces = { player: ['#stage', '#workspace', '.player-bar'] };
