...
const ICONS = {
  ...
  note: '<path d="M4.5 19.5h4L19.5 8.5l-4-4-11 11z"/>', dots: '<path d="M5.5 12h.5M11.75 12h.5M18 12h.5"/>',
};
...
.more-answer{margin-top:10px}
.more-answer>summary{cursor:pointer;min-height:44px;display:flex;gap:6px;align-items:center;font-size:13px;color:var(--ac)}
.more-answer[open]>summary{margin-bottom:6px}
...
.plan button{width:100%;text-align:left;border:1px solid var(--line);border-left:4px solid var(--sc);border-radius:10px;background:var(--surf);padding:8px 12px;min-height:44px;cursor:pointer;display:grid;gap:2px;font:inherit;color:var(--ink2)}
.plan b{display:flex;gap:6px;align-items:center;color:var(--sc);font:600 12px var(--mono);letter-spacing:.05em;text-transform:uppercase}
...
#understand #start-review{margin-top:6px;min-height:44px;font-size:14px}
...
#ov-top #start-review{display:none}
...
<header class="top">
  ...
  <div class="top-r">
    ...
    <details id="more"><summary class="btn" aria-label="More options">${I('dots')}More</summary><div class="more-body">
      ...
      <div class="row"><button class="btn" type="button" data-only="#slot-proto">Only the prototype</button><button class="btn" type="button" data-only="">Show all four</button></div>
      <h3>Your answers</h3>
...
${review.brief ? `      <section id="brief" aria-labelledby="brief-h">
      ...
      <button class="btn pri" type="button" id="start-review">${I('right')}Start</button>
    </div></div>
...
${review.allowAddedItems === false ? '' : `      <section class="add" aria-labelledby="addh">
  ...
  <section id="places" aria-label="The four places">
    <section class="slot" id="slot-map" aria-labelledby="slot-map-h"><div class="slot-head min-head"><span class="num">1</span><h2 id="slot-map-h">${I('map')}Map</h2>
      <button class="btn min" type="button" data-min="#slot-map" aria-expanded="true" aria-label="Minimise the map">Minimise</button></div>
    ...
    <section class="slot" id="slot-proto" aria-labelledby="slot-proto-h"><div class="slot-head min-head"><span class="num">2</span><h2 id="slot-proto-h">${I('phone')}Prototype</h2>
      <button class="btn min" type="button" data-min="#slot-proto" aria-expanded="true" aria-label="Minimise the prototype">Minimise</button></div>
    ...
    <section class="slot" id="slot-expected" aria-labelledby="slot-expected-h"><div class="slot-head min-head"><span class="num">3</span><h2 id="slot-expected-h">${I('list')}What should happen</h2>
      <button class="btn min" type="button" data-min="#slot-expected" aria-expanded="true" aria-label="Minimise what should happen">Minimise</button></div>
    ...
    <section class="slot" id="slot-build" aria-labelledby="slot-build-h"><div class="slot-head min-head"><span class="num">4</span><h2 id="slot-build-h">${I('code')}How I'd build it</h2>
      <button class="btn min" type="button" data-min="#slot-build" aria-expanded="true" aria-label="Minimise how I'd build it">Minimise</button></div>
...
const noteOpen = new Set(), ovOpen = new Set();
document.addEventListener('toggle', e => {
  const d = e.target, id = d.dataset && d.dataset.for; if (!id) return;
  const set = d.classList.contains('more-answer') ? noteOpen : d.classList.contains('ov-places') ? ovOpen : null;
  if (set) set[d.open ? 'add' : 'delete'](id);
}, true);
...
function renderTour(){
  ...
  const p = it ? placesFor(it) : tourAt === 'start' ? startPlaces() : {};
...
// Understand: what is coming, one row per part, each a jump.
function startPlaces(){
  return { expected: '<p class="qprompt">' + I('list') + 'What I will ask</p><ul class="plan">' + segs().map((s, i) => '<li><button type="button" data-seg="' + i
    + '" style="--sc:' + s.color + '"><b>' + I(s.icon) + esc(s.label) + '</b><span>' + s.ids.length + (s.ids.length === 1 ? ' question' : ' questions') + '</span></button></li>').join('') + '</ul>' };
}
...
function answerHtml(it, sec){
  ...
  const noteShown = cur !== 'unset' || store.notes[it.id] || (store.pictures || []).some(p => p.on === it.id) || noteOpen.has(it.id);
  return \`<fieldset class="item\${chosen ? ' chosen' : ''}\${it.step ? ' open' : ''}" data-v="\${cur}" style="--tone:var(--\${TONE[(picked || {}).tone] || 'line2'})">
    ...
    \${asksRow(it.id)}
    <details class="more-answer" data-for="\${esc(it.id)}"\${noteShown ? ' open' : ''}><summary>\${I('note')}\${cur === 'unset' ? 'Add a note or a picture' : 'Say why, in your own words'}</summary>
...
function placesFor(it, overview){
  ...
  return {
    ...
    expected: (it.step ? outcomeCols(it) : '') + (flds ? \`<div class="flds">\${flds}</div>\` : '') + examplesHtml(it.examples),
    build: (it.step ? buildHtml(it) : '') + (it.ref ? \`<p class="ref">\${esc(it.ref)}</p>\` : ''),
  };
}
...
function renderSelection(){
  ...
  $('#selection-actions').innerHTML = edge ? '' : (selection?.commentTarget ? '<button class="btn" id="comment-selected" type="button">' + I('note') + 'Comment on the map</button>' : '')
