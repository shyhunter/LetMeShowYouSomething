...
const KIND_ICON = { 'user-action': 'person', notification: 'bell', timer: 'clock', wait: 'clock', deadline: 'clock', schedule: 'calendar',
...
function outline(kind, x, y, w, h) {
  ...
  switch (kind) {
    ...
    case 'subflow': return r(6) + `<path class="dg-line" d="M${x + 8} ${y}v${h}M${x + w - 8} ${y}v${h}"/>`;
...
export function drawDiagram(diagram, opts = {}) {
  ...
  for (const e of L.edges) {
    ...
    parts.push(`<g class="dg-edge dg-e-${escSvg(e.kind)}${e.back ? ' dg-back' : ''}" data-from="${escSvg(e.from)}" data-to="${escSvg(e.to)}">`
      + `<path d="${d}" marker-end="url(#dg-arrow-${id})"/></g>`);
  ...
  for (const [nodeId, box] of Object.entries(L.nodes)) {
    ...
    parts.push(`<g data-node="${escSvg(nodeId)}" class="dg-node dg-k-${escSvg(n.kind)}${selected ? ' dg-selected' : ''}${partClass(nodeId)}"`
      + (linked ? ` data-step="${escSvg(n.step)}" tabindex="0" role="button"` : '')
