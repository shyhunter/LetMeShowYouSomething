...
import { aiNodeHeight } from './draw-ai.mjs';
...
export function layout(diagram) {
  ...
  const nodeH = n => aiNodeHeight(n) || (n.component ? COMPONENT_H : NODE_H);
  const cellH = members => Math.max(96, ...members.map(n => nodeH(n) + 26));
  ...
  for (const n of nodes) {
    boxes[n.id] = { x: colX[col[n.id]], y: bandY[laneOf(n)] + PAD + sub[n.id] * bandCell[laneOf(n)],
      w: NODE_W, h: nodeH(n), col: col[n.id], row: sub[n.id], lane: laneOf(n) };
  }
