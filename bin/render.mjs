...
.say-more,.note-btn{display:none}
.ptabs{display:none}
...
@container (max-width:700px){
  ...
  .say-more,.note-btn{display:inline-flex}
...
const DIAGRAMS = (FLOW ? [flowAsDiagram(REVIEW)] : []).concat(REVIEW.diagrams || []);
...
function chipsHtml(s){
  ...
  return c.length ? '<span class="chips">' + c.map(([k, t, i]) => '<span class="status ' + k + '">' + I(i, 'sm') + esc(t) + '</span>').join('') + '</span>' : '';
}
function sayFor(s){
...
function earlierHtml(s){
  if (s.kind !== 'item' || !(s.it.affects || []).length) return '';
  return s.it.affects.map(a => '<div class="earlier"><b class="h">' + I('history', 'sm') + 'Earlier · ' + esc(a.effect) + '</b><p>' + I('user', 'sm') + ' You answered "' + esc(a.decision.title) + '": <b>'
...
function mapHtml(s, zoom, d){
  ...
  const answered = new Set(REVIEW.items.filter(i => store.verdicts[i.id] || Object.values(store.choices).includes(i.id)).map(i => i.id));
...
function renderTour(){
  if (st.phase === 'return') return '<div class="tour">' + progressHtml(null)
    ...
    + '<div class="t-vis">' + slots({ map: mapHtml(null), proto: galleryHtml(''), expected: '<div class="expected">' + summaryRows() + '</div>', build: downloadsHtml() }, { proto: FLOW ? 'every screen' : '', expected: 'your answers', build: 'downloads' }) + '</div>'
  ...
  return '<div class="tour">' + progressHtml(s)
    ...
    + '<div class="rowgap"><button type="button" class="btn quiet say-more" data-act="why">' + I('info', 'sm') + 'Why I ask</button></div>' + earlierHtml(s) + '</header>'
...
function renderOverview(){
  ...
  return '<div class="ov"><div class="brief"><div class="m-head"><p class="eyebrow">' + I('flag', 'sm') + 'A review for you</p><button type="button" class="btn small" data-act="expand">' + I('expand', 'sm') + 'Expand</button></div>'
    ...
    + progressHtml(null) + (DIAGRAMS.length ? slotHtml('map', 'Map', 'map', 0, mapHtml(null), 'the whole review') : '') + body + missingHtml()
...
function layerHtml(){
  ...
  if (st.layer === 'why' && s) return '<div class="sheet-layer" role="dialog" aria-modal="true" aria-label="Why I ask"><div class="sh"><h3>' + I('info') + esc(stepTitle(s)) + '</h3><p class="say">' + sayFor(s) + '</p>' + earlierHtml(s) + '<button type="button" class="btn pri" data-act="close-layer">Got it</button></div></div>';
  return '';
}
