...
export function applyProposals(diagram, proposals) {
  ...
  for (const p of proposals) {
    ...
    if (d.agent === true && p.op === 'add-node') {
      no('agent additions need an explained continuation or stop. Describe the addition in a comment for the next review'); continue;
    }
...
// #77 — the first id with this prefix that nothing uses yet. `taken` must list every id an answer has
// or is pointed at by: the review's items (a carried `added-1` among them), added items, comments and
// the comments proposals name, or a new answer lands on an old one.
export function freeId(prefix, taken) {
...
export function buildFeedback(review, store, now = new Date().toISOString()) {
  ...
  // #33 — `screen`: the reviewer's screenshot of that screen, for the agent to use next round.
  const screens = new Set((review.flow?.screens || []).map((s) => s.id));
  const pictures = (store.pictures || []).filter((p) => onTitle[p.on] !== undefined).map((p) => ({
    id: p.id, on: p.on, onTitle: onTitle[p.on], ...(screens.has(p.screen) ? { screen: p.screen } : {}), type: p.type, width: p.width, height: p.height, data: p.data }));
