...
// #105 — a request is read when the file reaches the agent, and answered in its next round: the tip says so,
// on hover, and to a screen reader as the checkbox's description.
const ASKS = [['example', 'Show me an example', 'The agent sees this once you send the file back, and shows a real example in its next round.'],
  ['explain', 'Explain this', 'The agent sees this once you send the file back, and explains it again, differently, in its next round.']];
const asksRow = (id) => \`<div class="asks-row">\${ASKS.map(([kind, label, tip]) => \`<label class="ask" title="\${tip}"><input type="checkbox" title="\${tip}"
